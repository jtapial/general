...
parser = argparse.ArgumentParser(description="This is a wrapper to split the content of a PDB folder into chunks, and measure distances from small molecules residues to every residue in the PDB file, using 'drug_interface_extraction_biopython.py'")
...
for number,chunk in enumerate(in_chunks):
	...
	os.system("submitjob.py -n pdb_extraction -q short -l h_rt=5:00:00 -b y $SRC/vastdb_maps/mapping_files_creation/drug_interface_extraction_biopython.py {0} {1}".format(chunk_path,out_path))
